    ...
    <div className="space-y-6 sm:space-y-7">
      <header>
        ...
        <p className="mt-2 max-w-[52ch] text-[15px] leading-7 text-slate-300 sm:text-base">
          Monthly is the paid-beta default. Choose annual only if you want to pay the year upfront.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
        ...
        >
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            Includes 10 finalized agreements each month
          </p>
        </button>
        ...
        >
          {cadence === "annual" ? (
            ...
            <span className="inline-block rounded-full border border-slate-600 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-slate-300">
              Optional
            </span>
          )}
          ...
          <p className="mt-2 text-[15px] leading-7 text-slate-300 sm:text-base">
            {formatMoneyUsdWhole(annualCharge)}/year, paid upfront
          </p>
          <p className="mt-1 text-sm font-medium text-emerald-400">
            ≈ {formatMoneyUsdWhole(effectiveMo)}/mo · Save {formatMoneyUsdWhole(saveUsd)}/year vs monthly
          </p>
          <p className="mt-3 text-sm leading-7 text-slate-300">
            Includes 10 finalized agreements each month
          </p>
          <p className="mt-2 text-sm leading-7 text-slate-400">{CHECKOUT_ANNUAL_WORKFLOW_LABEL}</p>
        </button>
      </div>

      <p className="text-center text-[15px] font-medium leading-7 text-slate-200 sm:text-left sm:text-base">
        LawDog Pro includes 10 successfully finalized premium agreements each month — revise, send, and sign on
        your timeline. Quota does not roll over.
      </p>

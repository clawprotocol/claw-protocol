...
import {
  type CreateFlowProductionPhase,
  isCreateFlowPastCapture,
  isGuidedFinalReviewPhase,
  isUpdatedAgreementReadyPhase,
} from "./createFlowTypes";
...
import { GuidedAgreementUpdateSummary } from "./guidedDealCompletion/GuidedAgreementUpdateSummary";
import { GuidedReviewFlowBanner } from "./guidedDealCompletion/GuidedReviewFlowBanner";
import { GuidedSigningTrustStrip } from "./guidedDealCompletion/GuidedSigningTrustStrip";
import {
...
import {
  ...
  logGuidedStateTransition,
} from "./guidedDealCompletion/guidedUxStability";
import {
...
import {
  ...
  logGuidedFinalReviewExplicitUnlockBlocked,
  logGuidedFinalReviewExplicitUnlocked,
  logGuidedFinalReviewExplicitUnlockStarted,
...
import {
  ...
  processReviewEditedVersionUpload,
} from "./reviewEditedVersionUpload";
import {
  logProReviewSigningFlowState,
  readEditedVersionIntent,
  resolveProReviewSigningFlowState,
} from "./proReviewSigningFlowState";
...
function CreateFlowSendRecipientsPanel({
  ...
  return (
    ...
    >
      ...
      {guidedSigningTrustSlot ? <div className="mt-3">{guidedSigningTrustSlot}</div> : null}
...
const AgreementBuilderIntake: React.FC<Props> = ({
  ...
  const [guidedAuthVersionNonce, setGuidedAuthVersionNonce] = useState(0);
  const [guidedPacketSendBlocked, setGuidedPacketSendBlocked] = useState(false);
  const [reviewUploadBusy, setReviewUploadBusy] = useState(false);
  const [reviewUploadError, setReviewUploadError] = useState<string | null>(null);
  const [reviewUploadTick, setReviewUploadTick] = useState(0);
  ...
  const runPersistedRefineFromStepBuffer = React.useCallback(
    async (
      ...
      opts?: {
        ...
        /** Guided bulk regeneration: do not flip displayPhase/loading (avoids editing_pro + isGenerating). */
        suppressGlobalGeneratingUi?: boolean;
      },
    ): Promise<boolean> => {
    ...
    setHardError(null);
    const suppressGlobalUi = opts?.suppressGlobalGeneratingUi === true;
  ...
  const advancePaidProToRecipientSetup = useCallback(() => {
    ...
    setCreateUiStage(CreateUiStage.DRAFT);
    setMobileWorkspacePane("preview");
  }, [createFlowPhase, guidedCompletionPhase, guidedFinalReviewExplicitlyOpened]);
  ...
  applyProRefineOutputToProSurfaceRef.current = (outRaw, opts) => {
    ...
    setDraft((prev) =>
      prev
        ? {
            ...prev,
            ...buildAuthoritativeReviewDraftPatch(normalized),
          }
        : null,
    );
    ...
    setPreviewPaneRevealed(true);
    setReviewDocRefreshTick((n) => n + 1);
    const patchedDraftForSync = draft
      ? ({ ...draft, ...buildAuthoritativeReviewDraftPatch(normalized) } as ParsedDraftShape)
      : null;
    logGuidedAuthoritativeReviewSync({
      authoritativePlain: normalized,
      renderedPreviewPlain: patchedDraftForSync ? buildPreviewForCurrentTier(patchedDraftForSync) : "",
      reviewDraft: patchedDraftForSync,
      pickerPlain: normalized,
    });
    if (scroll) {
      window.requestAnimationFrame(() => {
        scrollLikelyReviewSectionIntoView();
  ...
  const guidedProUxState = useMemo(() => {
    const state = resolveGuidedProUxState({
      premiumPaidDocumentSurface,
      hasGuidedSession: Boolean(guidedCompletionSession ?? guidedCompletionSessionRef.current),
      guidedCompletionPhase,
      createFlowPhase,
      premiumRecipientUxActive,
      finalReviewExplicitlyOpened: guidedFinalReviewExplicitlyOpened,
  ...
  const proRefineCurrentDocumentTextForProPanels = useMemo(() => {
    ...
    if (guidedFinalReviewActive && authBody.length >= 500) return authBody;
    ...
    return pickAuthoritativeProCorpusForRefine({
      ...
      premiumReadonlyPlain: premiumPaidReadonlyPick.plainText || authBody,
  ...
  const unifiedPrimaryCta = useMemo((): PrimaryCtaState => {
    ...
    if (createProductionTwoPane) {
      ...
      {
        const guidedSessionForEarlyCta =
          guidedCompletionSession ?? guidedCompletionSessionRef.current;
        const pendingEarly = guidedSessionForEarlyCta
          ? countUnresolvedRenderableVariables(guidedSessionForEarlyCta)
          : 0;
        const guidedEarlySticky = resolveGuidedProStickyCta(
          guidedProUxState,
          pendingEarly,
        ...
        if (
          ...
          if (guidedEarlySticky.reason === "signer_setup_ready_final_review") {
            ...
            const placement = resolveGuidedFinalReviewCtaVisibility({
              signerSetupActive: guidedProUxShowsSignerSetup(guidedProUxState),
              signerSlotsComplete: guidedPreReviewSignerSlots.complete,
              applyStatus: resolvedGuidedAnswerApplyStatus,
              bulkApplying: guidedBulkApplyingActive,
              stickyBottomBarVisible: stickyForSigner,
              finalReviewUnlocked: guidedFinalReviewExplicitlyOpened,
            });
            if (placement.showInline && !placement.showSticky) {
              return {
                label: "",
                action: "guided_continue",
                disabled: true,
            ...
            if (!placement.showSticky && !placement.showInline) {
              return {
                ...guidedEarlySticky,
                disabled: true,
              };
      ...
      if (createUiStage === CreateUiStage.DRAFT) {
        {
          ...
          const pending = guidedSessionForCta
            ...
            : 0;
          const guidedSticky = resolveGuidedProStickyCta(
            guidedProUxState,
            pending,
            guidedPreReviewSignerSlots.complete,
            resolvedGuidedAnswerApplyStatus,
  ...
  const effectivePrimaryCtaDisabled =
    (simpleCreateUnifiedBottomCta ? unifiedPrimaryCta.disabled : primaryIntakeCtaDisabled) ||
    guidedPacketSendBlocked;
  ...
  const guidedSessionKey = useMemo(() => {
    const snap = readPremiumCompletionSnapshot();
    const genId = snap?.agreementGenerationId ?? getOrInitSessionAgreementGenerationId();
    const fp = shortIntakeFingerprint((currentPremiumMergedIntakeKey || intakeCombined).trim());
    return buildGuidedSessionKey(String(genId), fp);
  }, [
    currentPremiumMergedIntakeKey,
    intakeCombined,
    premiumSurfaceGateTick,
  ...
  const handleGuidedSaveAnswer = React.useCallback(
    (
      variableId: string,
      ...
      setGuidedCompletionSession((prev) => {
        ...
        if (isGuidedCompletionComplete(keyed)) {
          ...
          guidedCompletionSessionRef.current = frozenReady;
          setGuidedCompletionSession(frozenReady);
          persistGuidedSession(frozenReady, guidedSessionKey);
      ...
      const causality = reinforceGuidedQueuedUpdate({
        variableId,
        displayAnswer,
        phase: "queued",
  ...
  const handleGuidedEditAnswer = React.useCallback(
    (variableId: string) => {
      ...
      setGuidedCompletionSession((prev) => {
        if (!prev) return prev;
        const next = clearGuidedAnswer(prev, variableId, bodyLen);
        const keyed = { ...next, sessionKey: guidedSessionKey };
        persistGuidedSession(keyed, guidedSessionKey);
        guidedCompletionSessionRef.current = keyed;
        return keyed;
  ...
  const handleGuidedBulkApply = React.useCallback(async (options?: {
    ...
    const stableBefore =
      ...
      lastPremiumWinningCorpusRef.current ||
      paidBodyForGuidedCompletion;
    updateLastKnownGoodAuthoritativeDraftRef(lastKnownGoodAuthoritativeDraftRef, stableBefore, "guided_bulk_start");
    const intakeText =
      (proRefineIntakeTextForProPanelsRef.current || "").trim() ||
      currentPremiumMergedIntakeKey ||
      intakeCombined;
    ...
    let ok = false;
    try {
      ok = await runPersistedRefineFromStepBuffer(instruction, {
    ...
    if (applyOutcome.status === "applied") {
      ...
      const versionSnap = bumpAuthoritativeAgreementVersion(
        nextLen,
        persistedPremiumReviewTitle || draft?.title || "",
      );
      ...
      flashGuidedAgreementUpdateToast(applyResult.toast, null);
      bumpPremiumSurfaceGateTick();
      setReviewDocRefreshTick((n) => n + 1);
  ...
  const handleGuidedOpenFinalReview = React.useCallback(() => {
    ...
    if (guidedFinalReviewExplicitlyUnlockedRef.current && guidedFinalReviewExplicitlyOpened) {
      logGuidedFinalReviewNavigationDeduped();
      return;
    }
    ...
    if (!gate.ok) {
      if (gate.reason === "signers_incomplete") {
        ...
        logGuidedFinalReviewExplicitUnlockBlocked(gate.reason);
      }
      if (gate.reason === "signer_field_focused") {
        logBlockedAutoNavigationWhileSignersEditing("open_final_review");
      }
      return;
    }
    guidedFinalReviewNavigationInFlightRef.current = true;
    logGuidedFinalReviewExplicitUnlockStarted();
    try {
      flushSync(() => {
        guidedFinalReviewExplicitlyUnlockedRef.current = true;
        setGuidedFinalReviewUnlockedAt(Date.now());
        setGuidedCompletionPhase("applied");
    ...
    resetPremiumRecipientsSurfaceForFinalReview,
    buildGuidedFinalReviewUnlockGateArgs,
    flushGuidedSignerMetadataBeforeFinalReview,
  ...
  React.useEffect(() => {
    if (!guidedProUxShowsUpdatedReadyCard(guidedProUxState) && guidedCompletionPhase !== "applied") return;
    if (
    ...
    if (
      ...
      logRecipientSetupPhaseBlocked("guided_ready_phase_revert", guidedProUxState);
      if (!guidedFinalReviewExplicitlyOpened) {
        setCreateFlowPhase("updated_agreement_ready");
        setPremiumRecipientUxActive(false);
      }
    }
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      logRecipientSetupPhaseBlocked("effect_phase_revert", guidedProUxState);
      if (guidedCompletionPhase === "applied" && !guidedFinalReviewExplicitlyOpened) {
        setCreateFlowPhase("updated_agreement_ready");
      } else if (guidedFinalReviewExplicitlyOpened) {
        setCreateFlowPhase("guided_final_review");
      }
    }
  }, [
  ...
  const guidedPacketStaleState = useMemo(
    () =>
      resolveSigningPacketStale({
        currentVersionId: guidedAuthoritativeVersionId,
        currentBodyHash: guidedAuthoritativeBodyHash,
      }),
    [guidedAuthoritativeVersionId, guidedAuthoritativeBodyHash],
  );

  const guidedSigningPacketStale = guidedPacketStaleState.stale;
  const guidedSigningPacketPreparedCurrent = Boolean(
    guidedPacketStaleState.preparedVersion &&
      !guidedSigningPacketStale &&
      guidedAuthoritativeVersionId,
  );

  const guidedPacketBlocksSend = Boolean(
    guidedSigningPacketStale && guidedPacketStaleState.preparedVersion,
  );

  React.useEffect(() => {
    setGuidedPacketSendBlocked(Boolean(guidedPacketBlocksSend));
    if (!guidedPacketBlocksSend || !guidedPacketStaleState.reason) return;
    logGuidedPacketStaleHardStop(guidedPacketStaleState.reason);
  }, [guidedPacketBlocksSend, guidedPacketStaleState.reason]);

  React.useEffect(() => {
    const id = agreementIdForReview?.trim();
    if (!id || !isAgreementPacketPrepared(id)) return;
    const versionId = readAuthoritativeAgreementVersion()?.versionId;
    if (versionId) {
      markSigningPacketPreparedAtGuidedVersion(versionId, guidedAuthoritativeBodyHash);
    }
  }, [
    agreementIdForReview,
    guidedAuthVersionNonce,
  ...
  const openPaidProPostInlineSendDestination = React.useCallback(async () => {
    ...
    if (
      ...
      if (!assertSigningSendReadyOrBlock()) return;
  ...
  const guidedFinalReviewCtaPlacement = useMemo(
    () =>
      resolveGuidedFinalReviewCtaVisibility({
        signerSetupActive: guidedPreReviewSignerSetupActive,
        signerSlotsComplete: guidedPreReviewSignerSlots.complete,
        applyStatus: resolvedGuidedAnswerApplyStatus,
        bulkApplying: guidedBulkApplyingActive,
        stickyBottomBarVisible: guidedStickyBottomBarVisibleForSignerCta,
        finalReviewUnlocked:
    ...
    [
      ...
      resolvedGuidedAnswerApplyStatus,
      guidedBulkApplyingActive,
      guidedStickyBottomBarVisibleForSignerCta,
      guidedFinalReviewExplicitlyOpened,
    ],
  );
  const showGuidedFinalReviewInlineCta = guidedFinalReviewCtaPlacement.showInline;
  ...
  React.useEffect(() => {
    if (guidedFinalReviewExplicitlyOpened || guidedFinalReviewExplicitlyUnlockedRef.current) return;
    if (guidedCompletionPhase !== "ready_to_apply") return;
    if (createFlowPhase === "signer_setup_required") return;
    setCreateFlowPhase("signer_setup_required");
    setPremiumRecipientUxActive(false);
  }, [guidedCompletionPhase, createFlowPhase, guidedFinalReviewExplicitlyOpened]);
  ...
  const guidedUxFlags = useMemo(
    () =>
      deriveGuidedUxPhaseFlags({
        showPrimaryGuided: showPrimaryGuidedCompletion,
        hasSession: Boolean(activeGuidedCompletionSession),
        phase: guidedCompletionPhase,
        hasAuthoritativeSummary: guidedAuthoritativeSummaryAreas.length > 0,
        guidedFinalReview: guidedFinalReviewActive,
      }),
    [
  ...
  React.useEffect(() => {
    logGuidedStateTransition(guidedUxFlagsRef.current, guidedUxFlags, guidedCompletionPhase);
    guidedUxFlagsRef.current = guidedUxFlags;
  ...
  const guidedSigningTrustSlot = useMemo(() => {
    ...
    return (
      <GuidedSigningTrustStrip
        agreementAuthoritativelyUpdated
  ...
  const guidedSuppressSecondaryDocActions = Boolean(
    guidedUxFlags.guidedApplying ||
      (guidedUxFlags.guidedUpdated &&
        !paidProInlineSignersReady &&
        guidedAuthoritativeSummaryAreas.length > 0),
  );

  const premiumRecipientSendHelper = useMemo(() => {
  ...
  const proReviewActiveStep = React.useMemo(
    () =>
      resolveProReviewActiveStep({
        ...
        guidedFinalReview: guidedFinalReviewActive,
        signersReady: paidProInlineSignersReady,
        packetPrepared: guidedSigningPacketPreparedCurrent,
      }),
    [
      guidedCompletionActive,
      guidedCompletionPhase,
  ...
  const uploadedSourceForReview = useMemo(() => {
    void reviewUploadTick;
    return agreementIdForReview ? readUploadedSourceDocument(agreementIdForReview) : null;
  ...
  const handleProReviewApplySuggestEdits = React.useCallback(async () => {
    const instruction = proReviewSuggestEditsDraft.trim();
    if (!instruction) return;
    setProReviewSuggestEditsBusy(true);
    setProReviewSuggestEditsError(null);
    const ok = await runPersistedRefineFromStepBuffer(instruction, {
      scrollToReview: true,
      suppressGlobalGeneratingUi: true,
    });
    setProReviewSuggestEditsBusy(false);
    if (!ok) {
      setProReviewSuggestEditsError("Could not apply those suggestions. Try shorter, specific edits.");
      return;
    }
  ...
  const proReviewSigningFlowState = useMemo(() => {
    ...
    return resolveProReviewSigningFlowState({
      uploadedSource: uploadedSourceForReview,
      editedIntent: readEditedVersionIntent(agreementIdForReview),
      packetPrepared,
      packetStale: guidedSigningPacketStale,
      signersReady: paidProInlineSignersReady,
      guidedApplied: guidedCompletionPhase === "applied",
  ...
  React.useEffect(() => {
    if (!showSimplifiedProReviewSigningFlow) return;
    logProReviewSigningFlowState(proReviewSigningFlowState);
  }, [showSimplifiedProReviewSigningFlow, proReviewSigningFlowState]);

  const handleReviewEditedVersionFile = React.useCallback(
    async (file: File) => {
      const id = agreementIdForReview?.trim();
      if (!id) return;
      setReviewUploadBusy(true);
      setReviewUploadError(null);
      const result = await processReviewEditedVersionUpload({ agreementId: id, file });
      setReviewUploadBusy(false);
      if (!result.ok) {
        setReviewUploadError(result.userMessage);
        return;
      }
      const snap = readPremiumCompletionSnapshot();
      if (snap) {
        persistPremiumCompletionSnapshot({
          ...snap,
          uploadedSourceDocumentText: result.text,
          premiumDraft: {
            ...snap.premiumDraft,
            uploaded_source_document_text: result.text,
          },
        });
      }
  ...
  const handleKeepLawDogVersionAfterUpload = React.useCallback(() => {
    const id = agreementIdForReview?.trim();
    if (id) logReviewEditedVersionKeptAsReference(id);
  }, [agreementIdForReview]);

  const handleCompareUploadedChanges = React.useCallback(() => {
    const id = agreementIdForReview?.trim();
    const text = uploadedSourceForReview?.text?.trim();
    if (!id || !text) return;
    writeUploadedSourceDocument(id, {
      text,
  ...
  const executePrimaryCta = (cta: PrimaryCtaState) => {
    ...
    void (async () => {
      try {
        ...
        if (cta.action === "guided_continue" && !cta.disabled) {
          if (cta.reason === "updated_agreement_ready") {
            handleGuidedOpenFinalReview();
            return;
          }
          if (
                              ...
                              <>
                                ...
                                >
                                  ...
                                  <div className="w-full max-w-[850px] rounded-sm border border-stone-200/90 bg-[#faf7f0] text-left text-stone-900 shadow-[0_1px_2px_rgba(0,0,0,0.05),0_22px_48px_-8px_rgba(15,23,42,0.28)] ring-1 ring-black/[0.07]">
                                    {guidedPreReviewSignerSetupActive ? (
                                      <div
                                        ...
                                        data-guided-final-review-sticky-cta={
                                          guidedFinalReviewCtaPlacement.showSticky ? "visible" : "hidden"
                                        }
                                        data-guided-final-review-inline-cta={
                                          showGuidedFinalReviewInlineCta ? "visible" : "hidden"
                                        }
                                        data-guided-final-review-unlocked-at={guidedFinalReviewUnlockedAt ?? ""}
                                      >
                                    ...
                                    ) : showGuidedPreReviewApplying ? (
                                      <div
                                        className="px-[clamp(1.35rem,4.5vw,2.65rem)] py-3.5 sm:py-4"
                                        data-testid="guided-pre-review-applying"
                                        role="status"
                                        aria-live="polite"
                                      >
                                        <p className="text-sm font-semibold text-stone-900">{GUIDED_APPLYING_HEADLINE}</p>
                                        <p className="mt-1 text-[11px] leading-relaxed text-stone-600">
                                          {formatGuidedApplyingSubcopy(guidedPreReviewAnswerCount)}
                                        </p>
                                        {guidedAppliedSummaryChecklist.length > 0 ? (
                                          <ul className="mt-3 space-y-1.5">
                                            {guidedAppliedSummaryChecklist.map((label) => (
                                              <li
                                                key={label}
                                      ...
                                      <div className="px-[clamp(1.35rem,4.5vw,2.65rem)] py-3.5 sm:py-4">
                                        <GuidedUpdatedAgreementReadyCard
                                          ...
                                          applying={guidedCompletionPhase === "applying_all"}
                                          applyError={guidedBulkApplyError}
                                          onReviewUpdatedAgreement={handleGuidedOpenFinalReview}
                                          onRetryApply={
                                    ...
                                    {premiumReviewDocEditorOpen ? (
                                      ...
                                      <>
                                        {(guidedUxFlags.isGuidedCompletion || guidedUxFlags.guidedReviewReady) &&
                                        (guidedUxFlags.guidedApplying || guidedUxFlags.guidedUpdated) ? (
                                          <div className="mx-[clamp(1.35rem,4.5vw,2.65rem)] mb-2 space-y-2">
                                            <GuidedReviewFlowBanner
                                              guidedActive
                                              phase={guidedCompletionPhase}
                                              signersReady={paidProInlineSignersReady}
                                            />
                                            {guidedUxFlags.guidedUpdated &&
                                            guidedAuthoritativeSummaryAreas.length > 0 ? (
                                              <GuidedAgreementUpdateSummary
                                                areas={guidedAuthoritativeSummaryAreas}
                                                variableIds={guidedAuthoritativeSummaryVariableIds}
                                                className="sticky top-2 z-10"
                                                primaryFocus={!paidProInlineSignersReady}
                                              />
                                            ) : null}
                                          </div>
                                        ) : null}
                                        ...
                                        <details ref={customInstructionSectionRef} className="mt-3 group">
                                          ...
                                          <div className="relative mt-2">
                                            <VoiceAugmentedTextArea
                                              ...
                                              placeholder={GUIDED_CUSTOM_INSTRUCTION_PLACEHOLDER}
                                              aria-label="Additional details for LawDog"
                                            />
                                          </div>

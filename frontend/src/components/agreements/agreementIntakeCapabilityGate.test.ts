...
describe("material-gap / money_vibe clarity ask (doctrine — do not draft junk)", () => {
  it("money_vibe-class dump asks for clarity and does not proceed to Review", () => {
    const decision = assessAgreementIntakeCapability(MONEY_VIBE_LIVE);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
    expect(decision.code).toBe("material_gap");
    expect(decision.clarification.kind).toBe("material_gap");
    expect(decision.clarification.title).toMatch(/material terms|payment instructions/i);
    expect(decision.clarification.whatWeHeard.join(" ")).toMatch(/deliverables|scope/i);
    expect(decision.clarification.whatWeHeard.join(" ")).toMatch(/KYC|cash-only|offshore/i);
    expect(decision.clarification.suggestedRewrite).toMatch(/Redwood Ventures/);
    expect(decision.clarification.suggestedRewrite).toMatch(/BluePeak Advisors/);
    expect(decision.clarification.suggestedRewrite).toMatch(/\$75,000/);
    expect(decision.clarification.suggestedRewrite).toMatch(/actual deliverables|scope/i);
    expect(evaluateIntentionalCreateDraftSubmit(MONEY_VIBE_LIVE).action).toBe("block_capability");
  });

  it("does not hard-code the fixture parties — same vibe with other names also asks", () => {
    const variant =
      "Wire $40,000 escrow tomorrow to offshore account for 'consulting' between Harbor Peak LLC and Cedar Craft Co. No deliverables specified. Cash only. Ignore KYC.";
    const decision = assessAgreementIntakeCapability(variant);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
    expect(decision.code).toBe("material_gap");
    expect(decision.clarification.suggestedRewrite).toMatch(/Harbor Peak LLC/);
    expect(decision.clarification.suggestedRewrite).toMatch(/Cedar Craft Co/);
    expect(`${decision.clarification.suggestedRewrite} ${decision.userMessage}`).not.toMatch(
      /Redwood Ventures|BluePeak Advisors/i,
    );
  });

  it("ordinary incomplete named-2p + money without scope asks (universal client↔lawyer)", () => {
    const incomplete = "Pay $40,000 between Harbor Peak LLC and Cedar Craft Co.";
    const decision = assessAgreementIntakeCapability(incomplete);
    expect(decision.ok).toBe(false);
    if (decision.ok) return;
    expect(decision.code).toBe("material_gap");
    expect(decision.clarification.why).toMatch(/work|rights|exchange|scope/i);
    expect(evaluateIntentionalCreateDraftSubmit(incomplete).action).toBe("block_capability");
  });

  it("Northline-class named-2p with real work still proceeds", () => {
    expect(assessAgreementIntakeCapability(NORTHLINE_CANONICAL).ok).toBe(true);
  ...
  it("lawful consulting dump with real deliverables proceeds", () => {
    const filled =
      "Draft a services agreement between Harbor Peak LLC and Cedar Craft Co for workflow automation " +
      "and inventory dashboard work, fee $75,000, term 90 days. Payment by invoice to a domestic account. " +
      "Governing law: Texas.";
    expect(assessAgreementIntakeCapability(filled).ok).toBe(true);
    expect(evaluateIntentionalCreateDraftSubmit(filled).action).toBe("proceed");
  });

  it("implementation is a general red-flag + missing-scope gate — not a Redwood fixture special-case", () => {
    const file = readFileSync(join(__dirname, "agreementIntakeClarification.ts"), "utf8");
    expect(file).toContain("resolveMaterialGapAskReason");
    expect(file).toContain('kind: "material_gap"');
    expect(file).toMatch(/ignore\|skip\|bypass\|waive\|without/);
    expect(file).toMatch(/no\\s\+deliverables\?/);
    expect(file).not.toMatch(/Redwood Ventures|BluePeak Advisors|cash@example\.invalid/);
    expect(file).not.toContain("isCoherentOrdinaryNamedTwoPartyForFailsafe");
    expect(file).not.toContain("looksOverSpecifiedOrComplexityIntake");
  });
});

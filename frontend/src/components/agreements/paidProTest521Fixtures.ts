...
export const TEST521_TARGET_DOCUMENT_LEN = 10_528;

export const TEST521_INTAKE = TEST519_COLON_ROLE_DASHBOARD_CREATE_INTAKE;

export function test521Draft(): ParsedDraftShape {
  return test519Draft();
}

const TEST521_GOOD_FAITH_CLAUSE =
  "The parties shall perform their obligations in good faith and in accordance with this agreement";

export function buildTest521SubstantiveDegradedDocumentBody(
  targetLen = TEST521_TARGET_DOCUMENT_LEN,
): string {
  ...
  while (body.length < targetLen) {
    body += `\n${i}. Supplemental Provision. ${TEST521_GOOD_FAITH_CLAUSE} in connection with section ${i}.`;
    i += 1;
  }
  return body.slice(0, targetLen);
}

export function buildTest521DegradedJsonParseDocumentTextOnlyWire(): PremiumFullDraftResult {
  const valid = buildTest521SubstantiveDegradedDocumentBody();
  return {

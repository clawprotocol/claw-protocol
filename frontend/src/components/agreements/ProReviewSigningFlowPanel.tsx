...
export function ProReviewSigningFlowPanel({
  ...
  return (
    ...
    >
      ...
      {packetStale ? (
        <p className="mt-2 rounded-md border border-amber-300/80 bg-amber-50 px-2.5 py-2 text-[11px] font-medium text-amber-950" role="alert">
      ...
      {showEditedActions && showUploadOptions ? (
        ...
        >
          <p className="text-xs font-semibold text-stone-900">Edited version received</p>
          {uploadedSource?.fileName ? (
            <p className="text-[11px] text-stone-600">{uploadedSource.fileName}</p>
          ) : null}
          <p className="text-[11px] leading-relaxed text-stone-600">
            Uploaded version saved. Full redline comparison is coming soon. You can use this version for
            signing or continue with the LawDog draft.
          </p>

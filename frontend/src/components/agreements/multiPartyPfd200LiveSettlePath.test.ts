...
import {
  ...
  shouldAcceptLeftoverNumberedOrgAfterUnusedBind,
...
/** Clean 3p leftover overlay: all three filled names already appear in the 200 corpus. */
const LIVE_THREE_NAMED = ["Solo Design", "CodeNest LLC", "BrightPay Ops"] as const;
...
describe("live pipeline sites after pfd 200 + leftover 2-party overlay", () => {
  ...
  /**
   * Live 3p leftover overlay after #203: generate already named all three filled
   * party-prep rows, leftover 2-party opening still has [ORG_1]/[ORG_2], unused-bind
   * is a no-op. Tip a6d79736 fail-closes leftover [ORG_n] even though the corpus
   * already names N=3 — restore #201/#202 accept-when-corpus-names-N≥3.
   */
  function buildLiveThreePartyLeftoverOrgOverlayCorpus(targetLen = 15_381): string {
    const names = LIVE_THREE_NAMED;
    const leftoverOpening =
      'This Agreement is entered into by and between [ORG_1] ("Client") and [ORG_2] ("Service Provider").';
    const amongNamed = `The commercial parties to this Agreement are ${names[0]}, ${names[1]}, and ${names[2]}.`;
    const notices = names
      .map((n) => `If to ${n}:\nEmail: [EMAIL]\nAddress: [ADDRESS]\n`)
      .join("\n");
    const signatures = names
    ...
    const head = [
      ...
      "",
      "1. SCOPE OF SERVICES",
      `${names[0]}, ${names[1]}, and ${names[2]} shall perform the professional services described in this Agreement.`,
      "2. PAYMENT",
      "Fees total the amount stated in the intake and are payable in monthly installments.",
      "3. TERM",
      "The term begins on the Effective Date and continues for the stated duration unless earlier terminated.",
  ...
  it("N=3 leftover overlay with all filled names already in corpus settles after unused-bind no-op", () => {
    const leftover = leftoverTwoPartyDraft([LIVE_THREE_NAMED[0], LIVE_THREE_NAMED[1]]);
    const intake = mergePartyPrepIntoCreateSubmitText(THREE_PARTY_DUMP, [...LIVE_THREE_NAMED]);
    const overlaid = {
      ...leftover,
      parties: LIVE_THREE_NAMED.map((name) => ({ name, role: "party" })),
    };
    const corpus = buildLiveThreePartyLeftoverOrgOverlayCorpus();
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});

    expect(corpus).toMatch(/\[ORG_1\]/);
    expect(corpus).toMatch(/\[ORG_2\]/);
    for (const name of LIVE_THREE_NAMED) {
      expect(corpus).toContain(name);
    }
    // Extract can miss filled names without an entity suffix (BrightPay Ops) — that
    // is the #203 commerciallyNamed hole. Accept must use filled-name corpus match.
    expect(extractAuthoritativeLegalNamesFromCommercialCorpus(corpus).length).toBeLessThan(3);
    expect(extractAuthoritativeLegalNamesFromCommercialCorpus(corpus)).not.toContain(
      "BrightPay Ops",
    );
    ...
    expect(bindOnly.bound, "unused-bind must be a no-op when all 3 names are already named").toBe(
      false,
    );
    expect(bindOnly.text).toMatch(/\[ORG_1\]/);
    expect(bindOnly.text).toMatch(/\[ORG_2\]/);

    const helperWithoutDetail = shouldAcceptPaidProCommercialFieldStubsAfterPfd200({
      text: corpus,
      intakeRaw: intake,
    });
    expect(
      shouldAcceptLeftoverNumberedOrgAfterUnusedBind({
        text: corpus,
        intakeRaw: intake,
        partyNames: [...LIVE_THREE_NAMED],
      }),
    ).toBe(true);
    const leftoverFatals = analyzeTemplatePlaceholderFragments(corpus, {
      intakeRaw: intake,
      partyNames: liveRejectPartyNames(overlaid),
    }).filter((d) => d.fatal && /\[ORG_[12]\]/.test(d.token));
    expect(leftoverFatals, "leftover [ORG_n] must be demoted on the live reject path").toEqual([]);
    ...
    expect(
      acc.ok,
      `helperWithoutDetail=${helperWithoutDetail} reject=${acc.ok} reasons=${acc.reasons.join("|")}`,
    ).toBe(true);
  ...
  it("thin corpus leftover [ORG_n] with no unused names and corpus not naming N≥3 fail-closes", () => {
    const intake = mergePartyPrepIntoCreateSubmitText(THREE_PARTY_DUMP, [...LIVE_THREE_NAMED]);
    const noUnusedThin = [
      "SERVICES AGREEMENT",
      "",
      "This Agreement is entered into by and between [ORG_1] and [ORG_2].",
      "",
      `${LIVE_THREE_NAMED[0]}, ${LIVE_THREE_NAMED[1]}, and ${LIVE_THREE_NAMED[2]} shall perform.`,
      "x".repeat(200),
    ].join("\n");
    const noUnusedBind = bindUnusedFilledPartyNamesIntoLeftoverOrgSlots(
      noUnusedThin,
      [...LIVE_THREE_NAMED],
      intake,
    );
    expect(noUnusedBind.bound).toBe(false);
    expect(noUnusedThin.length).toBeLessThan(8_500);
    expect(extractAuthoritativeLegalNamesFromCommercialCorpus(noUnusedThin).length).toBeLessThan(3);
    expect(
      shouldAcceptLeftoverNumberedOrgAfterUnusedBind({
        text: noUnusedThin,
        intakeRaw: intake,
    ...
    const acc = rejectPremiumBodyForProRender(noUnusedThin, {
      intakeText: intake,
      partyNames: [...LIVE_THREE_NAMED],
    });
    expect(acc.ok).toBe(false);

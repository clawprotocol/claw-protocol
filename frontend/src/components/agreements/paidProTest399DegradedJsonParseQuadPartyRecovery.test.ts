...
import { meetsPaidProDegradedRecoveryDisplayRequirements, PAID_PRO_RECOVERY_MIN_DISPLAY_LEN } from "./paidProPostCheckoutRenderGate";
...
function buildTest399RejectedDegradedBody(targetLen = 10_244): string {
  const header = [
...
beforeEach(() => {
  ...
  (globalThis as { __paidProAllowStructuralRetryInTest?: boolean }).__paidProAllowStructuralRetryInTest =
    true;
  h.callIndex = 0;
  const degraded = buildTest399RejectedDegradedBody();
  h.mockResults = [degradedJsonParseResult(degraded), degradedJsonParseResult(degraded)];
});

describe("TEST399 degraded json_parse quad-party recovery", () => {
  it("deterministic local recovery satisfies Pro display gates for test396 intake", () => {
    const draft = test396Draft();
    ...
    expect(meetsPaidProDegradedRecoveryDisplayRequirements(localRecovery.body, TEST396_QUAD_PARTY_INTAKE)).toBe(
      true,
    );
    ...
    expect(collectForbiddenTemplateFragments(localRecovery.body, TEST396_QUAD_PARTY_INTAKE)).toEqual([]);
  ...
  it("both premium-full-draft attempts degraded/json_parse resolve via deterministic fallback", async () => {
    ...
    const out = await runPremiumCompletion({
      ...
      userGapAnswers: null,
      agreementGenerationId: "gen-test399",
      premiumRequestIntakeFingerprint: "fp-test399",
    ...
    expect(meetsPaidProDegradedRecoveryDisplayRequirements(out.winningPremiumBodyText, TEST396_QUAD_PARTY_INTAKE)).toBe(
      true,
    );
    ...
    const commit = tryCommitPostCheckoutRecoveryToPaidProSourceOfTruth({
      ...
      intakeText: TEST396_QUAD_PARTY_INTAKE,
      premiumRenderSource: PREMIUM_DEGRADED_SERVER_LOCAL_RECOVERY_RENDER_SOURCE,
      reviewSessionId: "test399-recovery",
    });

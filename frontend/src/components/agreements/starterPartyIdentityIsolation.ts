...
/** Structured intake field labels fused onto entity values ("Legal entity / party name: Acme LLC"). */
const LEADING_PARTY_METADATA_FIELD_LABEL_RE =
...
export function isolateLegalEntityFromContaminatedName(raw: string): string {
  ...
  // Field-label prefixes from Party N bullet manifests must not stick to the entity.
  for (let i = 0; i < 2; i += 1) {
    const stripped = s.replace(LEADING_PARTY_METADATA_FIELD_LABEL_RE, "").trim();
    if (stripped === s) break;
    s = stripped;
  }
  ...
  if (/\b(?:create|draft|prepare|write|generate)\s+(?:a|an)\s+\w+\s+agreement\b/i.test(s)) {
    const trailingEntity = s.match(
      /\b((?:[A-Z][\w.&'’-]+(?:\s+[A-Z][\w.&'’-]+)*\s+(?:LLC|L\.L\.C\.|Inc\.?|Corp\.?|Corporation|Ltd\.?|Limited|LP|L\.P\.|LLP|PLLC|Co\.|Company)))\.?\s*$/i,
    );
    if (trailingEntity?.[1]) {
      s = norm(trailingEntity[1]);
    }
  }

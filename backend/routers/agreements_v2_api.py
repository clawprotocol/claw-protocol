...
def _premium_full_draft_system_prompt() -> str:
    return (
        ...
        "**Notices stanza format (required when Notices section is included):** Each party must have its own separate block—never fuse multiple parties on one line. "
        "Use this structure per party, each block separated by a blank line:\n"
        "If to [Full Legal Entity Name]:\n"
        "[Full Legal Entity Name]\n"
        "Attn: [Signer Name], [Title]\n"
        "Email: [email if known]\n"
        "Address:\n"
        "[street line]\n"
        "[city, state zip if known]\n"
        "Do not emit inline patterns like `If to Party A : Party A If to Party B :`. "
        "Do not fuse the Notices section number into the prior sentence (e.g. never `performed.10. Notices`). "
        "Each numbered section heading must start on its own line.\n"
...
def post_completed_signed_export_pdf(
    ...
    """Download PDF for a fully executed signed agreement (owner/read auth; canonical signed snapshot only)."""
    from backend.services.completed_signed_pdf_export import build_completed_signed_pdf_response
...
def get_public_completed_signed_export_pdf(agreement_id: str) -> Response:
    """Public PDF download for fully executed agreements only (canonical signed snapshot)."""
    from backend.services.completed_signed_pdf_export import build_completed_signed_pdf_response
